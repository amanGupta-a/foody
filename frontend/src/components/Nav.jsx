...
const Nav = () => {
  const { userData ,currentCity ,cartItems, itemsInMyCity, shopInMyCity } = useSelector(state => state.user)
  ...
  const [query, setQuery] = useState('')
   const handleLogOut = async() =>{
    ...
    const handleSearchItem = async()=>{
       ...
       try{
         const apiResults = currentCity
          ? await axios.get(`${serverUrl}/api/item/search-items?query=${encodeURIComponent(query.trim())}&city=${encodeURIComponent(currentCity)}` , {withCredentials:true})
          : { data: [] }

         const localItemMatches = Array.isArray(itemsInMyCity)
          ? itemsInMyCity.filter((item) => {
              const itemName = String(item?.name || '').toLowerCase()
              const itemCategory = String(item?.category || '').toLowerCase()
              return itemName.includes(normalizedQuery) || itemCategory.includes(normalizedQuery)
            })
          : []

         const matchedShops = Array.isArray(shopInMyCity)
          ? shopInMyCity.filter((shop) => {
              const shopName = String(shop?.name || '').toLowerCase()
              const shopCity = String(shop?.city || '').toLowerCase()
              const shopAddress = String(shop?.address || '').toLowerCase()
              return shopName.includes(normalizedQuery) || shopCity.includes(normalizedQuery) || shopAddress.includes(normalizedQuery)
            })
          : []

         const shopItems = matchedShops.flatMap((shop) =>
          Array.isArray(shop?.items)
            ? shop.items.map((item) => ({ ...item, shop: item?.shop || shop }))
            : []
         )

         const combined = [...(apiResults?.data || []), ...localItemMatches, ...shopItems]
         const uniqueItems = []
         const seen = new Set()
         ...
         dispatch(setSearchItems(uniqueItems))
       }
       catch(err){
         console.log(err)
       }
      ...
      {showSearch && userData.role== "user" &&  <div className="md:flex w-[70%] h-[60px] bg-white
        shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[15%] " > <div className="flex items-center w-[30%] gap-2 px-3 border-r">
         ...
         <div className="flex items-center w-full gap-2 px-3">
          <IoMdSearch size={22} className="text-[#ff4d2d] cursor-pointer" onClick={handleSearchSubmit} />
          <input
            type="text"
